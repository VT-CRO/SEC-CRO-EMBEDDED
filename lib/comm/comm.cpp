...
void parsePacket(Crobot_packet_handler_t* pkt_handler, const char* buff)
{
  ...
  uint16_t total_packet_length = PKT_NUM_HEADER_BYTES + \
                                      PKT_NUM_LEN_BYTES + \
                                      pkt_handler->rx_packet->data_packet_u.data_packet_s.len;

    uint16_t crc = MAKE_16B_WORD(
    ...
    uint16_t new_crc = updateCRC(0, pkt_handler->rx_packet->data_packet_u.data_packet, total_packet_length - PKT_NUM_CRC_BYTES);
...
void buildPacket(Crobot_packet_handler_t* pkt_handler, tendon_opcode_t opcode, uint8_t id, int numParams)
{
  ...
  pkt_handler->tx_packet->data_packet_u.data_packet_s.len = numParams + PKT_NUM_CRC_BYTES + PKT_NUM_ID_BYTES + PKT_NUM_OPCODE_BYTES;
